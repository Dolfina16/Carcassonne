...
public void heapify(int pos){
    if(tamaño > 2 && pos>1){
        if(elementos.get((pos-1)/2) == elementos.get((pos-2)/2)){
            siftDown(new int[]{(pos-1)/2,pos,pos-1});
            heapify(pos-=2);
        }else{
...
public void siftDown(int[] elems){
    ...
    if(elems.length == 2){
            if(comparador.compare(padre, elementos.get(elems[1])) < 1){
                elementos.set(elems[0], elementos.get(elems[1]));
                elementos.set(elems[1], padre);
            }
    }else if(elems.length == 3){
        int hijoMayor = comparador.compare(elementos.get(elems[1]), elementos.get(elems[2])) > 1 ? elems[1] : elems[2];
        if(comparador.compare(padre, elementos.get(hijoMayor)) < 1){
...
public static void main(String[] args) {
    ...
    Ciudad ciudad1 = new Ciudad(0);
    Ciudad ciudad = new Ciudad(1);
    Ciudad ciudad2 = new Ciudad(2);
    ciudad.incr_ganancia(200);
    ciudad1.incr_ganancia(10);
    ciudad2.incr_perdida(400);
    Heap<Ciudad> heap = new Heap<Ciudad>(new Ciudad[]{ciudad2,ciudad1,ciudad}, SuperavitComparator);
}
